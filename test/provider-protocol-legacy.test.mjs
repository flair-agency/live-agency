...
const rootDir = path.resolve(import.meta.dirname, "fixtures/installation");
    ...
    () =>
      validateProviderPackage(
        "@fixture/incompatible",
        {
          ...
          version: "1.0.0",
          peerDependencies: { "@flair-agency/provider-protocol": "^2.0.0" },
          liveAgencyProvider: {
            schemaVersion: 1,
...
// A malformed dot-prefixed entry must not bypass the explicit ./ manifest format.
test("provider module resources require the package-relative ./ prefix", () => {
  const manifest = entry => ({
    version: "1.0.0",
    peerDependencies: { "@flair-agency/provider-protocol": "^1.6.0" },
    liveAgencyProvider: {
      schemaVersion: 1, provides: [ACTIVITY_CAPABILITY],
      accepts: ["text/markdown"], execution: { kind: "module", entry },
    },
  });
  for (const entry of [".hidden/index.js", "../outside.js", "src/index.js"]) {
    assert.throws(() => validateProviderPackage("@fixture/path", manifest(entry), rootDir),
      error => error.code === "INVALID_MANIFEST");
  }
  assert.doesNotThrow(() => validateProviderPackage("@fixture/path", manifest("./src/index.js"), rootDir));
});
