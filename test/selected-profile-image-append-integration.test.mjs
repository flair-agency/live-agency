...
import { buildLarkBaseImageAppendIntent, createLarkBaseSelectedImageAppender, createLarkBaseHistoryMediaReader, LARK_BASE_API_OPERATIONS } from '../providers/lark-base/src/index.js';
...
async function fixture(t, writeOutcome = 'success') {
  ...
  const seed = await prepareProfilePlan({ client: reader, config, manifest, observations: observations(), nowMs: NOW });
  const [seedRow] = buildProfileCreateIntentRows({ plan: seed.plan, bindings: seed.bindings });
  c.state.history.push({record_id: 'recResumeProfile', fields: {...seedRow.fields, Creator: [{record_ids: seedRow.fields.Creator}]}});
  ...
  assert.equal(plan.summary.profileAttachExistingCount, 1);
  const intent = buildLarkBaseImageAppendIntent({ appendSelection, uploadSelection, tableId: config.profileTableId,
    avatarField: bindings.profile.avatar, baselineRecord: c.state.history[0], baselineAttachments: [],
    avatar: observation.creators[0].profile.avatar, planSha256: plan.planSha256 });
  const baselineFields = structuredClone(c.state.history[0].fields);
  const events = [];
  const writer = createLarkBaseSelectedImageAppender({ appendSelection, uploadSelection, readSelection: c.selection, mediaSelection,
  ...
  return { c, reader, plan, events, file, baselineFields, apply: overrides => applyProfilePlan({ client: { ...reader, ...writer }, config, reviewedPlan: plan,
    apply: true, expectSha256: plan.planSha256, confirmProfileCreate: 0, confirmProfileAttach: 1, ...overrides }) };
}

for (const outcome of ['success', 'lost']) test(`Profile missing-image resume and byte readback through selected CLI: ${outcome}`, async t => {
  const f = await fixture(t, outcome);
  const result = await f.apply();
  assert.equal(result.verified, true); assert.equal(result.profileCreatedCount, 0); assert.equal(result.profileAttachedCount, 1);
  assert.equal(result.recoveredFromAmbiguousResponse, outcome === 'lost');
  assert.equal(f.c.state.history.length, 1);
  const { Avatar, ...preserved } = f.c.state.history[0].fields;
  assert.deepEqual(preserved, f.baselineFields);
  assert.equal(f.c.state.calls.filter(args => args[2]?.endsWith('/append_attachments')).length, 1);
  assert.equal(f.c.state.calls.filter(args => args[2]?.endsWith('/records/batch_create')).length, 0);
...
test('missing attachment approval or changed local image produces no POST', async t => {
  for (const change of ['count', 'file', 'stale']) {
    const f = await fixture(t);
    if (change === 'file') fs.writeFileSync(f.file, 'changed');
    if (change === 'stale') {
      f.c.state.history[0].fields.Followers = 999;
      assert.equal((await f.apply()).status, 'blocked');
    } else await assert.rejects(f.apply(change === 'count' ? { confirmProfileAttach: 0 } : {}));
