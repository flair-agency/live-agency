...
| --- | --- | --- | --- | --- | --- |
| `live-agency-creator-invitation-eligibility-record` | Creator Scouting | Lark targets; normalized BackStage eligibility observation | Append or extend reviewed invitation-state history in Creator Scouting | Required for writes | Reads and dry runs are unattended-capable; every write still requires exact operation approval |
...
| `live-agency-creator-live-observation-record` | Creator Scouting, then Management consumer after handoff | Lark targets; normalized TikTok LIVE observation | Append LIVE history and metric snapshots | Required for writes | Interactive acquisition |
| `live-agency-creator-assessment-update` | Creator Scouting | Existing profile and LIVE evidence in Lark | Update reviewed insight and approved tags | Required for writes | Target read and dry-run only |
| `live-agency-creator-profile-history-prune` | Creator Scouting history maintenance | Existing Lark profile history | Delete exact approved redundant records under the owning backup contract | Explicit destructive approval | Dry-run only |
| `live-agency-creator-live-session-history-prune` | Creator Scouting history maintenance | Existing Lark LIVE history | Delete exact approved redundant records under the owning backup contract | Explicit destructive approval | Dry-run only |
| `live-agency-creator-live-metric-history-prune` | Creator Scouting history maintenance | Existing Lark metric history | Delete exact approved redundant snapshots | Explicit destructive approval | Dry-run only |
| `live-agency-creator-invitation-eligibility-history-deduplicate` | Creator Scouting history maintenance | Existing Lark invitation history | Archive, then delete exact approved adjacent duplicates | Explicit destructive approval | Dry-run only |
| `live-agency-data-backup-create` | Cross-domain data protection | Reviewed Lark backup route; shared storage receipts | Create and full-readback-verify an exact Base backup | Preauthorized exact scheduled destination; no deletion | Eligible when both Bindings support unattended execution |
| `live-agency-data-backup-retention-plan` | Cross-domain data protection | Verified backup artifacts and receipts | Produce retention reasons and deletion candidates; no storage deletion | Plan review; deletion belongs to a separate workflow | Dry-run only |
| `live-agency-data-recovery-test` | Cross-domain data protection | Verified backup; isolated test destination | Restore and reconcile outside production | Explicit test creation/cleanup authority | Preflight only |
| `live-agency-datastore-maintain` | Cross-domain data protection | Backup receipts, capacity counts, child dry-run summaries, retention and drill status | Coordinated maintenance plan; no direct mutation | Child-specific approval for every destructive action | Read, backup coverage, and dry-run coordination only |
...
| `live-agency-coin-purchase-expense-reconcile` | Expense and Accounting | Normalized purchase evidence and expense candidates | Reviewed one-to-one expense registration | Required | No general unattended mutation |
| `live-agency-weekly-coin-expense-claim-submit` | Expense and Accounting | Frozen prototype only | None while frozen; weekly applications remain manual | Not applicable | Do not invoke, activate, or schedule |
