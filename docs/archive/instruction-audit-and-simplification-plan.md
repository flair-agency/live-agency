...
The [remaining-work map](../governance/development-policy.md) and permanent policy
...
- [Task orchestration policy](../governance/development-policy.md): Owns permanent work policy and the accepted model default. Remaining stopping, splitting, and conflict rules are audit targets, not already simplified by plan approval.
- [Skill naming and migration plan](../reviews/v2-skill-naming-and-migration-plan.md): Renaming, distribution, and foreign-revenue relocation remain separate responsibilities. Share inventories/evidence when targeting the same Skill, while keeping changes and authority scopes explicit.
