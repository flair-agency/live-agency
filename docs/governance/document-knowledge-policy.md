...
The company original specifies Google Docs named version history and generally Japanese document names. Existing project documents use English paths and Git review/version history. The storage/version mapping for these project documents remains an explicit owner decision: keeping the current repository during this change does not establish an exception to the company original, nor authorize a bulk Google Docs migration. Preserve both originals and existing history until that mapping is decided. Do not treat a Git push as changing or versioning the company originals.
