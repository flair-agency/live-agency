...
For this project policy, the canonical location is flair-agency/live-agency, branch main, path docs/governance/document-knowledge-policy.md. On 2026-09-10, the owner requested main as the release baseline; main was created from the previously adopted codex/project-root baseline. Unmerged work remains proposed. The company document-management original remains the linked Google Doc. The former baseline branch is retained for history; existing open proposals target main. Existing project English naming follows the owner's language policy.
