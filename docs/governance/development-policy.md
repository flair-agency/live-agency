...
Use existing supported configuration inputs and explicit paths; keep actual machine values outside Git. The [Runtime configuration procedure](../../provider-runtime/docs/configuration.md) distinguishes CLI arguments, injected interfaces and proposed deployment mechanisms. Do not create a new configuration mechanism merely to make a document portable.
...
When dependency installation is in scope, use the owning lockfile with lifecycle hooks disabled (`npm ci --ignore-scripts`). The Runtime manifest's existing postinstall performs the child install; with hooks disabled, perform that declared child step explicitly only when needed: from Runtime, `npm ci --ignore-scripts --prefix skills/live-agency-skills`. This is a reproducible installation procedure, not evidence that installation ran. Review an actually needed build hook against the scoped build/release contract before running it; the old SEP instruction does not establish that every future hook is forbidden.
