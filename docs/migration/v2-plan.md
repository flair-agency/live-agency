...
| Finalize contracts and package mapping | Resolve cycles and inverted concrete dependencies; cover common libraries, all target Providers/Skills, Runtime and only necessary entry-point adapters; include formal names, exports and old identifiers |
...
Start the end-to-end check with one representative operation. Complete installation/resource resolution for the whole supported foundation package set before declaring this stage complete; full business acceptance belongs to Stage 3. Synthetic success does not verify a real Provider. Local tarballs are intermediate evidence, not a substitute for registry deployment and client connectivity.
...
1. Align instructions, code, resources and business logic.
...
Inspect the foreign-revenue instructions, scripts, references, profiles, callers and temporary receipt-monitor lifecycle. Stage 1 establishes separation, package installation and Runtime connectivity; Stage 2 verifies required real capabilities; Stage 3 accepts recognition/settlement and cuts over invocation/monitor references. Preserve monitor identity/ownership and reconcile pending state to avoid duplicate monitors or journal execution. Keep the original source until packaged invocation and rollback are verified. This migration does not certify accounting policy or create production journals merely to demonstrate relocation.
...
Stage 1 covers complete supported-package installation/resources and representative client connectivity. Stage 3 completes each supported Skill's installed business path and operational acceptance; these are not additional legacy M7/SN queues.
