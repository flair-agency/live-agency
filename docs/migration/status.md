...
is canonical; the [Japanese review](../reviews/profile-avatar-readback-adoption-ja.md)
is retained as approved review history. The approved scope selects TikTok Web
1.1.1, Lark Base 1.4.0-m3.4, platform 0.1.0-m3.3 and catalog 0.1.0-m3.4 for
private publication, verified new installation, the selected production Work
switch, and acquisition/read/planning for the previously selected single target.
Runtime, Profile, Transport, CLI and selected service configuration remain fixed.
The downloaded catalog must match the approved raw-byte SHA-256 before installation;
