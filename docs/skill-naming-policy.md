...
- Status: Reviewed policy; adoption is tracked in a separate migration plan.
- Created: 2026-09-05
...
belong in the [Skill naming and migration plan](v2-skill-naming-and-migration-plan.md).
Update that plan when implementation status changes; do not turn the policy
into a progress log or a legacy-name mapping.

- [Domain model](live-agency-domain-knowledge.md): the business vocabulary and invariants that names must represent.
