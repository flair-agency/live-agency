...
## 17. Lark ChatのM2：2025年への訂正と取得結果

対象年の訂正を受け、同じチャットを**2025年8月20日0時以上・23日0時未満（日本時間）**で再取得しました。指定Appのユーザーと法人を再確認し、配布済みLark Chat Provider 1.0.0のAPIアダプターと範囲制限付きtransportを使っています。

**2件取得でき、次ページはありませんでした。** システム通知1件とカード形式1件で、いずれも8月22日5時29分頃の投稿です。元の応答と変換結果について、メッセージ・チャット・送信者ID、時刻と期間、種類、構造化本文のハッシュ、スレッド参照、削除状態を照合し、一致を確認しました。システム通知の空の送信者IDは、既存仕様どおりnullに変換されています。

前回の0件は訂正前の2026年に対する結果として保存しています。画面との比較を求めた確認は不要になりました。今回の実データには通常テキスト・添付・スレッド・複数ページの例がないため、それらの実検証を完了扱いにはしません。

残る実装上の確認点は、製品のselected API経路が現在Bot用の仕様だけを検証済みとしていることです。今回はユーザーを明示したCLI接続によるアダプター単体の実接続・変換確認であり、Provider全体の選択から実行までの検証は残っています。

今回のメッセージ一覧要求は1回（前回を含め計3回）で、書込みはありません。製品コード・パッケージ・本番設定も変更していません。実チャットIDと応答はGit管理外に保管しています。
