...
## 17. Lark ChatのM2：指定期間が0件のため未完了

指定された1チャットについて、2026年8月20日0時以上・23日0時未満（日本時間）を読み取りました。既存Appのユーザーと法人の一致を確認しています。

- CLIのユーザー認証による取得：0件、次ページなし。
- GitHub Packagesから隔離導入したLark Chat Provider 1.0.0のAPIアダプターと範囲制限付きtransportによる取得：0件、次ページなし。

製品のselected API経路は現状Bot用の仕様だけを検証済みとしており、今回はユーザーを明示したCLI接続によるアダプター単体の実接続確認です。Provider全体の選択・実行経路が通ったとはしていません。公式API仕様の認証記載と今回のユーザー認証による成功応答との違いも残っています。

**0件だけでは、実際に投稿がないのか、APIから見える範囲が違うのかを判定できません。** 本文・添付の変換や複数ページ取得の実データ検証も未完了です。指定期間の投稿がLark画面上で見えるか、利用者に確認しています。

メッセージ一覧要求は計2回で、書込みはありません。パッケージ・本番設定の変更もありません。実チャットIDや応答はGit管理外の検証証跡に保存しています。
