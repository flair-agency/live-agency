---
type: design-review
visibility: internal
status: pending
date: 2026-09-11
author: "Naoki Kimura（方針指示）; Codex（文書化・ソース照合）"
context: "事業ドメイン・プラットフォーム固有ドメインと実装の責務を記録し、現在のLark読取障害との関係を区別する"
---

# ドメイン知識の実装案レビューと経緯

# 2026-09-13の知識配置方針

オーナーは、LIVEエージェンシー業をプラットフォームに共通する抽象ドメインとして`live-agency`に置き、その具象である各プラットフォームの知識を非公開リポジトリーに閉じ込めるよう指示した。Skillは抽象ドメインモデルとユースケースから導出し、内容の開示可否を確認したうえで公開する方針であり、Providerは含む知識により公開・非公開が分かれる。この指示を英語の[知識所有文書](../architecture/domain-knowledge-ownership.md)に反映する。配置方針は今回の明示指示で採用済みであり、本文のPR統合・既存知識の移管・公開設定の変更とは区別する。

2026-09-11の「両ドメインを親へ集約する」という方針を更新する。下記の旧説明と4つの実装案の図は当時の検討経緯として保存する。特に、公開予定の`live-agency`内にプラットフォーム固有ドメインを描いた旧パッケージ図は、今後の配置指示には使わない。本書全体の`pending`は残る実装案の状態であり、今回の明示指示を未決定とするものではない。

| 知識・成果物 | 配置と公開範囲 | 理由 |
| --- | --- | --- |
| LIVEエージェンシー業の抽象モデル・共通のユースケース | `live-agency`、公開可能な内容 | 共通業務を特定プラットフォームの制度や仕様から独立して説明する |
| プラットフォームの具象モデル・抽象との対応 | 非公開リポジトリー | NDA対象を含む知識を公開側へ伝播させず、同一プラットフォーム内の複数Providerが同じ意味を参照する |
| Skillの業務手順・判断・検証・引継ぎ | 抽象モデルとユースケースから導出する公開Skill | 特定画面の操作ではなく、業務目的と必要な中立の事実から手順を決める |
| Providerの取得・変換・サービス操作 | 含有知識により公開または非公開 | 公開可能な汎用操作と、保護される仕様の実装を区別する |
| 事業者固有の運用、実環境の対応設定 | 私有の運用文書・選択環境 | 一社の運用や実リソースを業界共通の不変条件にしない |

```mermaid
flowchart LR
  subgraph Public["公開可能な領域"]
    Model["LIVEエージェンシー業の抽象モデル"] --> UseCase["ユースケース"]
    UseCase --> Skill["公開Skill"]
    Generic["公開可能なProvider実装"]
  end
  subgraph Private["非公開領域"]
    Concrete["プラットフォームの具象知識"]
    Mapping["抽象との対応・根拠"]
    Provider["保護される知識を適用するProvider"]
    Concrete --> Mapping
    Provider -.参照.-> Concrete
    Provider -.参照.-> Mapping
  end
  Mapping -.対応先.-> Model
  Skill -.中立の機能を要求.-> Generic
  Skill -.選択済みの中立機能を要求.-> Provider
```

矢印は導出・対応・利用の関係を示し、npm依存や新APIの採用を意味しない。具象知識は非公開側で解釈し、公開Skillは正規化された事実を業務判断に用いる。公開Skillの説明は非公開仕様を読まずに理解できるものとし、実行に必要な私有知識・版・復旧手順は、権限のある人が選択済みProvider・環境の文書からたどれるようにする。秘密の内容やリンクを公開Skillへ埋め込んで引継ぎ経路を作らない。

## 情報資産としての判断と配置の順序

オーナーは、全てを非公開にすれば一定の情報保護は可能でも、知識の所属・共通性・固有性や保護理由への意識が抜けやすいと説明した。公開範囲を意識して配置を決めることは、情報資産・情報セキュリティの観点から、これらの判断を明示して点検できるようにする。その理由を正本へ反映し、さらに「共通性と秘匿性は別の軸」という評価も採用するよう指示した。

共通性は業界横断・一つのプラットフォーム内・複数Provider間・一社内などの適用範囲を表し、秘匿性は出所・契約上の制約・事業上の機密性に基づく開示可能な相手を表す。共通して使う知識でも秘匿が必要な場合があり、特定サービスの操作でも公開仕様に基づく実装は公開可能な場合がある。抽象化、固有名の除去、架空例への置換だけでは、元の意味や判断ロジックの保護理由は消えない。

配置は、1. 意味の所属と共通する範囲を特定、2. 開示可否と保護理由を独立に判断、3. 両方を満たすリポジトリーと参照経路を選択、の順で決める。根拠・未確認事項は既存の所有文書か作業記録へ残す。開示範囲が未確認なら適切な非公開の所有元で保持し、公開・非公開の都合で知識の所属を付け替えない。新しい台帳や分類名は設けない。

| 例 | 判断と配置 |
| --- | --- |
| 公開可能な根拠で説明できる業界共通ユースケース | 公開する抽象モデルと、そこから導出するSkillへ |
| 制限のある資料から導いた、複数プラットフォームに当てはまる抽象的な分析 | 共通性とは別に開示制約を確認し、制約が残る間は私有の業務知識として保持する |
| 公開可能な仕様だけで実装する特定サービスの操作 | Provider固有でも公開可能な場合がある |
| 複数Providerが参照するNDA対象のプラットフォーム規則 | 同一プラットフォーム内で共通でも、正本と適用実装は非公開の境界を保つ |

これは配置判断の例であり、実在の資料の公開許諾や新しいリポジトリーを選択した記録ではない。プラットフォーム具象知識を非公開に置く既存の方針は維持する。その方針を、全ての固有知識が法的に秘密であるという認定や、全Providerを非公開にする理由にはしない。正本の[判断手順](../architecture/domain-knowledge-ownership.md#information-assets-and-placement-decisions)を参照する。

## 既存知識の移行対象

以下は移行対象の棚卸しであり、移管済み一覧ではない。追跡は既存の[Issue #2](https://github.com/flair-agency/live-agency/issues/2)を使用し、同Issueの事業説明に関する未完了要件も残す。

| 対象 | 抽象側に残す意味 | 分離する知識と移管先の責務 |
| --- | --- | --- |
| 業務モデル §3.3、§4.1–4.4 | 所属確認後の引継ぎ、観測元と業務記録の権威の区別 | 特定プラットフォームの確認元・取得面・権威は非公開プラットフォーム文書へ。保存サービスの役割は対応定義へ |
| §4.5 識別・アカウント関係 | 識別の意味、継続性の不確かさ、観測と判断の区別 | プラットフォームIDの意味は非公開具象モデルへ。実フィールド・リンクは対応定義へ。既存の識別・招待ルールを名称置換で一般化しない |
| §4.7 LIVE時間 | 観測値と報酬計算の根拠となる値を区別する | ソースの収録範囲・月次値の意味は非公開具象モデルへ。差分キー・集計日付は業務上の根拠を確認し、Skillか私有運用の責務へ |
| §5–8 戦略・コイン・ギフト・支援 | 支出と推定消費額の区別、支援の目的 | プラットフォーム決済挙動は非公開具象モデルへ。特定事業者の目的分類や表計算による導出は私有運用へ。抽象として採用できる範囲を根拠ごと確認する |
| §9 指標・ROIとその他の運用上の選択 | 指標の意味、前提と限界の明示 | 一社の評価式・報酬制度・運用閾値を共通仕様にしない。プラットフォーム制度と事業者の判断を分ける |
| 構成・宣言・テスト・レビュー・履歴 | 公開可能な汎用構成と架空例 | private依存やリンク、保護される意味を残した架空例、過去のレビュー・Git履歴・配布物を公開前に点検する。文書の移動だけで公開可能とはしない |

## 移行順序と完了条件

| 順序・作業 | 人の担当 | 期限 | 内容と前提 | 完了条件・参照 |
| --- | --- | --- | --- | --- |
| 1. 配置方針の整合 | TBD | TBD | 今回の明示指示を正本・関連文書に反映する | scoped diff、ポリシー、参照を検証したPR。今回の作業範囲 |
| 2. 私有の所有元と移管対応の確定 | TBD | TBD | 1を基に、既存の非公開所有元または新設先を選び、移す節・根拠・消費側を対応付ける | 移管先のアクセス範囲、正本の版、参照元、保持対象が具体的。Issue #2 |
| 3. 知識の保存と参照の切替 | TBD | TBD | 2の選択後に私有側へ理由・例外・引継ぎ手順を保存し、読み戻して一致を確認してから旧配置を除く | 知識の欠落なし、公開抽象との対応あり、変更リンク・配布資源・影響する契約を検証。移管前の固定版を復旧用に保持 |
| 4. 公開可能性の確認 | TBD | TBD | 3の後に現行ツリーだけでなく履歴・付属成果物・公開依存を点検する | 保護される知識の露出経路と残る未確認事項を記録。実際の公開設定や履歴書換えは別の明示選択で実施 |

今回は1の文書変更を具体化する。非公開リポジトリーの最終名・共有知識の配置単位・マッピング方式は未選択であり、架空の移管先を作らない。GitHubの親リポジトリーは作業時点で`PRIVATE`を確認した。公開設定、契約上の公開可否、Git履歴の安全性は今回の作業で認定しない。

## 今回の作業とAIポリシーレビュー

主分類E（知識の所有・情報境界）、副分類G（文書）。対象は親の方針文書と既存のレビュー・移行記録。Skill/Providerの実装、子pin、環境、運用ルール、配布を変更しない。最初の作業単位の完了条件は、今回の指示に沿った一貫した文書差分と確認可能なPRである。次の作業は私有の移管先と節単位の対応を具体化すること。復旧はこの文書差分のrevertであり、移管元の知識や既存実装は保持される。

`origin/main`の`e4b4351`から`codex/platform-knowledge-boundary`を作成した。元checkoutの`runtime`変更は保持し、独立worktreeで編集する。独立レビュー担当（gpt-6-astra / low）は現行の矛盾箇所の探索を読取専用で担当した。担当間で共有ファイルを編集しない。既存のIssue #2を継続し、PRは今回の配置方針のみを追跡する。

AIレビューの根拠は今回のオーナー明示指示、文書知識・言語・開発方針、全文を確認したPrivate Source Integration Guideである。旧方針による「両ドメインを親へ集約」「Providerは一律非公開」という記述を修正し、採用済み方針と未移管の内容、公開方針と実際の設定、公開Skillの理解と私有の実行手順へのアクセスを区別した。新たな業務不変条件や私有ソースの意味は追加していない。

変更文書11ファイルの相対リンク167件を確認し、欠落0件。未展開コンポーネントの参照先8種類は親が指定する子commitのGit内容で確認した。scoped diffと空白検査、新しい配置図・抽象モデル図と本文の対応も確認した。独立レビューで追加の修正必須事項はなかった。GitHub上の図の描画、人による引継ぎ演習、子リポジトリーの全面監査・履歴の公開審査は未実施。文書と参照だけの変更でファイル移動や実装変更がないため、実行系テストと依存の導入は行っていない。

リモート同期の試行は自動承認レビューにより実行前に拒否された。理由は、NDA関連情報を含む可能性がある11文書について、具体的な内容とGitHubへの送信先を対象とした明示承認が不足しているという判定である。同期先`flair-agency/live-agency`の`PRIVATE`設定、変更内容のレビュー、既存の開発同期方針は確認済みだが、この拒否を迂回しない。ローカル変更を保存し、同じ非公開リポジトリーの`codex/platform-knowledge-boundary`へのpushとPR作成、既存Issue #2・Projectへの記録を送信前の確認対象とした。その時点ではPR・Issue・Projectは未更新であり、文書差分の準備完了とリモート同期待ちを区別した。後続のチケット化の明示依頼と実施結果は下記に記録する。

同日の追加指示では、情報資産として公開範囲を意識する理由と、共通性・秘匿性の独立判断を同じ文書作業へ反映する。元の作業単位と保護境界を維持し、正本・知識方針・配布方針・本記録・移行状況を更新する。追加前のローカルcheckpointは`e1f123b`。AIレビューでは今回の明示指示を根拠に、「抽象化したから公開できる」と読める3箇所を修正し、保護対象の抽象知識も私有の業務所有元へ置けることと、既存の具象知識配置方針を区別した。リモート送信への回答としては扱わず、同期保留は継続する。

追加変更5文書の相対リンク85件と空白差分を確認し、欠落・不備0件。未展開のRuntime文書への参照は親が指定する子commitで確認した。独立レビュー（同じgpt-6-astra / low担当）は情報境界・例示・判断順序を確認し、修正必須の問題なし。図・実装・配布資源の変更はなく、実行系テストは行っていない。追加差分はローカルコミットとして保存する。復旧はこの追加コミットのrevertで、前の配置方針のcheckpointへ戻せる。

## GitHub Projectsへのチケット化

オーナーの「この件、GitHub Projectsのチケットとして」という明示依頼により、既存の[Issue #2](https://github.com/flair-agency/live-agency/issues/2)へ採用済み方針・理由・独立した判断軸・移管の完了条件を追記した。主分類Gの追跡更新である。元の本文は旧状態の見出しを明確にした以外保持し、重複チケットは作成していない。[LIVE Agency Maintenance](https://github.com/orgs/flair-agency/projects/2)の既存項目を`Intake / triage`に設定した。次の作業は私有の正本所有元と節単位の移管先を具体化すること。人の担当・期限はTBDである。

既存Draft [PR #3](https://github.com/flair-agency/live-agency/pull/3)は以前の説明拡充だけを扱うため、本文の`Closes #2`だけを通常の関連参照へ変更した。拡張したIssueを部分対応PRだけで自動完了にしない。IssueはOPEN、PRはOPEN/Draftを保持し、本文2件の読み戻し一致、Project所属・状態、項目数10件の維持、PRのclosing relationshipから#2が除かれたことを確認した。独立担当は送信前の本文・元の範囲の保持・公開境界を確認し、修正必須事項なし。

明示依頼は当該チケットと関連付けの更新に適用した。その時点では文書ソースのpush、新しいPR作成、公開設定変更、実際の知識移管は実施していなかった。元の本文とProject状態はローカル作業出力に保持し、修正が必要な場合は後続の変更を確認してから当該追記・状態変更だけを戻す。ローカルの状況記録も更新し、チケット化完了とその時点のソース同期待ち・移管未着手を区別した。

## 方針文書のPR化と移行の分離

オーナーが`domain-knowledge-ownership-ja.md`等のPR化を明示的に求め、さらに「ポリシーの文書化と実態の整合は別で進める」と指示した。この依頼に基づき、同じ文書11ファイルを非公開の`flair-agency/live-agency`の専用ブランチへ同期し、配置ポリシーだけを扱うPRを作成する。前回の送信保留はこの追加の明示依頼で更新される。主分類GのPR準備・追跡を、既存のE/G文書作業の完了まで含める。

今回のPRは方針・理由・判断手順・関連文書の整合を採用する単位であり、これらの文書が統合されれば完了する。実際の知識移管、既存成果物・履歴の公開審査、公開設定の変更はIssue #2に残し、文書PRの完了条件にしない。具体的な私有所有元・移管先・方式が未選択でも方針PRは進められる。PRは#2を関連参照とし、自動完了させない。

PR準備では最新main `474f764`をローカルブランチへ取り込む。`docs/migration/status.md`先頭の競合は、今回の方針記録とmainの既存リリース記録を両方保持して解消する。mainのcatalog・platform README更新はそのまま保持し、今回のPR差分には含めない。既存の内容レビューを再利用し、独立担当はPR説明と範囲の分離を確認した。自動workflowはCIのみがpush/PRで動き、パッケージ公開は手動起動のままである。

最新mainとの差分は方針文書11ファイル。統合後の相対リンク171件（子commitで確認した8種類を含む）、空白差分、競合マーカーの除去、mainの既存記録の保持を確認した。実装・pin・配布資源の変更はなく、実行系テストの追加実施は行っていない。

同期commit `bb2c2e3`から[方針PR #70](https://github.com/flair-agency/live-agency/pull/70)を`main`向けに作成し、LIVE Agency MaintenanceへPR自体を登録した。レビュー時の状態はOPEN・通常PR・MERGEABLEで、期待する11文書だけが差分に含まれる。Issue #2にはPRと文書化・実態整合の分離を反映し、本文の読み戻し一致、Issue/PR両方のProject所属、#2を自動完了させる関係がないことを確認した。CIは作成時点で実行中であり、成功済みとは報告していない。main統合・移管完了は未実施のまま保持する。

# この記録の位置付け

以下は2026-09-11時点の説明・実装案・検証記録である。知識の配置については上の2026-09-13の指示が優先する。当時の障害と次の作業も、現在の状態や実行選択を示すものではない。

オーナーから「この整理をちゃんと文書化しておいて。これを踏まえて、今直面したバグは解決しそうですか？」との指示を受けて記録する。採用済み方針の正本は英語の[知識所有文書](../architecture/domain-knowledge-ownership.md)（`status: commit`）である。本書はその参照説明と経緯を含む、未決定の実装案のレビュー文書（`status: pending`）とする。クラス名・宣言的なマッピング方式・コードの配置は本書で検討し、採用済み方針を再定義しない。未実装・未検証の部分を承認済み仕様や本番の状態として扱わない。

5図のうち、ドメインモデルは採用済み正本に、設計クラス図・パッケージ図・コンポーネント図・配置図は本書の未決定案に保存する。後半3図はMermaidによるUMLの簡略表現である。業務上の概念、実装インターフェース、ソース配置、実行関係、環境配置をそれぞれ別の図にし、文書への参照をコード依存や実行と区別した。

# 正本の採用済み方針と理由（参照）

| 整理 | 理由・保つ境界 |
| --- | --- |
| LIVEエージェンシー事業ドメインと、TikTokなど各LIVEプラットフォーム固有ドメインを区別する | Scouting／Managementは業務の区分、TikTokはプラットフォームの区分であり、片方で他方を置き換えない |
| ドメインモデルを含む知識の正本は`live-agency`の文書に集約する | 複数の実装が意味を別々に定義せず、人が根拠を読んで業務を引き継げるようにする |
| ユースケースごとにSkillが対応する | Skillが業務の進行・判断・計画・適用される承認・結果検証を持つ。Runtimeは選択済み環境へのアクセスを支える |
| 業務の論理スキーマ、Larkへの対応定義、実環境のIDを分ける | Lark Base ProviderにCreator Scouting／Creator Managementの業務定義を押し込めない |
| TikTokの共通知識、共通の取得処理、Web／iOS／BackStage固有の操作を区別する | 共通する意味を重複定義せず、画面や権限が異なる取得経路を同一視しない |
| ソース公開方針は`live-agency`・Skill・Runtime・Lark関連が公開、BackStageが非公開 | TikTok Web／iOSは非公開候補のまま。実際の公開変更、レジストリー配布、本番導入は別の作業・判断である |

知識を文書に集約しても、SkillやProviderが実装時にその知識を使わなくなるわけではない。正本を参照し、その意味を実装に反映する。認証画面の手順・未公開仕様・私有の証跡は、公開予定のドメイン文書へ転記せず、権限のある人が参照できるProvider側の知識として保持する。

招待可否は、Providerが返すステータスと招待区分を、Skillが内部の子ステータスへ対応させる整理を維持する。今回の文書化で新たなステータスや全件停止条件は追加しない。共通Creator UUIDや新しいマスターテーブルの導入、ScoutingとManagementの同一レコード化も行わない。

# 未決定の設計案

2026-09-13追記：招待の読取・計画接続については、[PR #73](https://github.com/flair-agency/live-agency/pull/73)のLGTMにより、汎用Lark読取・私有の項目対応設定・Skillの業務判断という方式を採用した。正本は[Record dataset read contract](../architecture/record-dataset-read-contract.md)。以下は以前の案を保持するもので、今回採用した読取方式を未決定へ戻したり、Profile接続の同時改修を招待の前提にしたりしない。その他のパッケージ・API案の未決定状態や、本書全体の`pending`は維持する。

以下の図は2026-09-11の案を保持する。親への具象知識集約は今回の指示で失効している。新しい配置には上の図と移行順序を使い、旧図の公開境界を実装しない。

`SchemaBinding`は、論理スキーマの版、保存先の参照、フィールド・リンクの対応を宣言として表す候補である。業務判断や任意の実行コードを設定へ埋め込むものではない。既存の型検証・正規化・検索・添付・関連付けを、この定義と汎用操作だけで表せるかは確認が必要である。

独立したドメイン実装パッケージや、業務別の保存Provider／アダプターパッケージを必須にする案は採用していない。まずプロフィール1ユースケースで、必要な責務と既存処理との対応を確かめる。具体的なAPI名、再利用するマッピング定義の配置、文書の版と実装の版の結び方、TikTok Web／iOSの最終的な公開範囲は残る判断である。

## 設計クラス図（案）

プロフィール1ユースケースのインターフェース案であり、クラス名・メソッド名をAPIとして採用するものではない。実装関係は責務を示す。手順を返すProviderは、AIがホストのツールで実行する場合もあり、必ずしもJavaScriptメソッドの実行を意味しない。

```mermaid
classDiagram
  class ProfileRecordSkill {
    preparePlan()
    verifyResult()
  }
  class RuntimeAccess {
    resolve(environment, capability)
  }
  class ProfileSource {
    <<interface>>
    observe(target)
  }
  class DataStore {
    <<interface>>
    readRecords(query)
    applyPlan(plan)
  }
  class TikTokWebProvider
  class TikTokIOSProvider
  class LarkBaseProvider
  class SchemaBinding {
    logicalSchemaVersion
    resourceReference
    fieldMappings
  }
  ProfileRecordSkill ..> RuntimeAccess : 選択した機能
  ProfileRecordSkill ..> ProfileSource : 中立の観測
  ProfileRecordSkill ..> DataStore : 中立のデータ操作
  TikTokWebProvider ..|> ProfileSource
  TikTokIOSProvider ..|> ProfileSource
  LarkBaseProvider ..|> DataStore
  LarkBaseProvider --> SchemaBinding : 選択した対応定義
```

Skillは必要な業務上の事実を知り、Larkのfield IDやTikTokの画面構造は持たない。図は一般的な更新・削除権限を付与しない。同じインターフェースに見えても、Providerごとの対応範囲・条件が同一とは限らない。具体的なインターフェースの所有者と、対応定義だけで処理を表せるかは未確認である。

## パッケージ図（案）

Mermaidのグループを使ったUMLの簡略表現。実線は実装依存、点線は文書または宣言の参照であり、文書参照を実行コードのimportとは扱わない。汎用capability contractへの依存は省略している。公開・非公開の表示は方針または候補であり、現在のGitHub設定を示さない。

```mermaid
flowchart TB
  subgraph LA["live-agency：公開方針"]
    Business["事業ドメイン文書"]
    PlatformDomain["LIVEプラットフォーム固有ドメイン文書"]
    Schema["論理スキーマ文書"]
    Mapping["サービス対応定義：配置案"]
    Platform["Platform宣言・カタログ"]
  end
  Skill["ユースケース別Skill：公開"]
  Runtime["Runtime：公開方針"]
  Lark["Lark Provider：公開方針"]
  WebIOS["TikTok Web・iOS：非公開候補"]
  BackStage["BackStage：非公開"]
  Skill --> Runtime
  Skill -.-> Business
  Skill -.-> Schema
  Platform -.-> PlatformDomain
  Runtime -.-> Platform
  WebIOS -.-> PlatformDomain
  BackStage -.-> PlatformDomain
  Mapping -.-> Schema
  Lark -.-> Mapping
```

実ID・認証情報はソースの外へ置く。RuntimeのPlatform参照は宣言の読み取りであり、全プラットフォームのインストールを要求しない。具体的なマッピング定義の配置は案である。[配布方針](../architecture/distribution.md#source-repository-visibility-and-synchronization)がソースの公開方針を規定し、レジストリー上のパッケージ公開範囲は別に判断する。

## コンポーネント図（案）

実線は呼出し・アクセス、点線は知識・構成の参照を示す。Runtimeは選択済みの操作や手順をSkillへ提供し、業務を開始する責務はSkillが持つ。

```mermaid
flowchart TB
  User["ユーザー"] --> Skill["ユースケース別Skill"]
  Skill -.-> Business["事業ドメイン文書"]
  Skill --> Runtime["Runtime：選択済み機能へのアクセス"]
  Runtime -.-> Environment["選択済み環境・固定版"]
  Environment -.-> Platform["Platform宣言"]
  Platform -.-> Domain["選択したプラットフォームのドメイン文書"]
  Skill -.-> Domain
  Runtime --> Source["選択した取得Provider"]
  Source -.-> Domain
  Source --> External["選択したLIVEサービス・操作面"]
  Runtime --> Lark["Lark Base Provider"]
  Lark -.-> Mapping["選択したSchemaBinding：方式案"]
  Lark --> Base["選択したBase"]
```

取得は必要なcapabilityに応じてWeb・iOS・BackStageを選ぶ。3経路すべてを呼ぶ図ではない。LarkのAPI優先・ブラウザへの順次フォールバックという採用方針は維持し、選択された権限と既知の操作結果を扱う。既存のBase／Chatでフォールバックが実装・検証済みであることは、この図だけでは示さない。

## 配置図（案）

利用者のローカル端末での実行・配布物の配置を示し、AI推論基盤の配置図ではない。プロジェクトの分離自体をセキュリティ境界とは扱わない。

```mermaid
flowchart TB
  subgraph GitHub["版管理されたソース"]
    Public["公開方針：ドメイン文書・Skill・Runtime・Lark"]
    Private["非公開：BackStage／非公開候補：TikTok Web・iOS"]
  end
  Public --> Artifacts["版を持つパッケージ・知識参照・構成"]
  Private --> Artifacts
  subgraph LocalHost["利用者のローカル端末"]
    subgraph Development["Codex開発環境"]
      Branch["ソースの作業ブランチ"] --> DevInstall["開発用の配置"]
      DevInstall --> DevState["開発用設定・認証参照・記録"]
    end
    subgraph Production["指定したChatGPT Work Local本番プロジェクト"]
      Invoke["インストール済みSkillを呼び出す"] --> ProdInstall["固定版Skill・Runtime・Provider・知識参照"]
      ProdInstall --> ProdState["本番用設定・認証参照・記録"]
    end
  end
  Artifacts --> DevInstall
  Artifacts --> ProdInstall
  DevState --> DevBase["開発用Base"]
  ProdState --> ProdBase["選択した本番Base"]
  ProdState --> LiveService["選択したLIVEアカウント・アクセス経路"]
```

開発変更は本番のSkill登録や設定を暗黙に上書きしない。ソース同期・パッケージ配布・インストール・業務受入は別の段階であり、1ユースケースの受入を全Skillの再配布完了まで待たせない。具体的な配置方式はこの設計案の確認対象である。

# 現在の読取障害との関係

**責務の混在を解消する方向は定まったが、この整理だけで現在のエラーが直るとはまだ言えない。** 業務スキーマの定義・環境の対応設定・Larkの実操作を区別すると修正対象を特定しやすくなる。一方、実際のAPI呼び出しや設定検証の失敗原因は、別途絞り込む必要がある。全コンポーネントの再設計・再配布を先に完了させる必要はない。

| 分かっていること | 判断できる範囲 |
| --- | --- |
| 最新の限定履歴読取診断は`read-fields`で停止した | この段階にはクリエイターテーブルの列定義読取、プロフィール履歴テーブルの列定義読取、両者の対応検証が含まれる |
| 上位には`PROFILE_ENVIRONMENT_FAILED`、内側には`PROFILE_DATASTORE_READ_FAILED`が出る | 異なる二つの根本原因ではなく、失敗を異なる層で包んだ結果。外側の名前だけでは環境設定の誤りと断定できない |
| Providerは`stage`を残すが元の例外理由を失い、Skill側でもその詳細が上位へ残らない | どちらのAPI呼び出し、またはフィールド・型・リンク対応検証で失敗したかを現状の出力から特定できない |
| この限定診断では履歴レコード読取・画像処理へ進んでいない | 今回の停止を履歴全件走査や添付処理の遅さと断定できない。以前の長時間読取とは分けて扱う |
| 先行するクリエイター読取は成功している | 後続の別テーブル・別処理の成功を保証しない |

ソース照合対象はLark Providerの`src/creator-profile-read.js`、`src/creator-profile-normalization.js`とProfile Skillの`src/profile-environment.mjs`。`read-fields`が2回の列定義読取と`resolveProfileFields`を包んでいること、Providerでの理由欠落と上位伝達の不足を確認した。認証・通信の問題、選択設定と実テーブルの不一致、実装側の検証不備は、現段階では原因候補である。

本番はChatGPT Work Localの指定プロジェクト「C|OPS|エージェンシー運営」。停止時の固定構成はRuntime `2.0.0-m3.0`、Profile `2.0.0-m3.1`、Lark `1.4.0-m3.0`。インストールとSkill登録は完了したが計画は未完成、`businessWorkflowVerified: false`、業務データ書込み・画像アップロードは0件である。Workの最終試行はクレジット不足で終了している。これは停止時の記録であり、今回再実行した結果ではない。

停止時の根拠は[本番検証の証跡checkpoint](https://github.com/flair-agency/live-agency/commit/ad3c1ba)と、その記録が参照する私有の`work-final-summary.json`・`work-diagnostic-summary.json`。実ID・クリエイター情報・生レスポンスはこの文書に転記しない。[Issue #32](https://github.com/flair-agency/live-agency/issues/32)が業務受入未完了を引き続き追跡する。

# 最小の次の作業

| タイトル | 人の担当 | 期限 | 内容 | 完了条件 | 参照 |
| --- | --- | --- | --- | --- | --- |
| 読取失敗の原因を特定する | TBD | TBD | 列定義読取2回と対応検証を区別し、秘密情報を除いた理由コード・段階を上位へ残す。選択済み環境・対象の限定読取で原因を1回切り分ける | 設定・サービス呼出し・実装のどこを修正するか証拠で説明できる | Issue #32、上記ソース |
| 必要箇所を修正してプロフィールの計画を作る | TBD | TBD | 特定した原因の修正に限定し、指定済みの1ユースケースを再開する | 計画・件数を実データから提示できる。登録完了とは区別する | Issue #32 |
| マッピング方式を具体化する | TBD | TBD | 既存のプロフィール処理と宣言的な対応定義の適合性を照合する | 再利用範囲・不足・版管理を具体的に説明できる。必須パッケージを推測で増やさない | 設計文書の未決定事項 |

この文書化の作業ではサービス再試行や実装変更を行わない。上の診断と実装は次の作業として残す。既に与えられた権限を尊重し、同一の承認を改めて要求する根拠にはしない。

# 作業範囲と検証記録

主分類G（文書）。作業単位は、採用済みの知識所有方針と5図を保存し、関連する正本の責務・公開方針の記述を整合させ、現在の障害との関係を記録すること。実装・契約・子リポジトリーのpin・配布・本番設定を保護境界とする。完了条件は内容と参照の照合、文書ポリシーレビュー、所有する文書差分のリモート作業ブランチへの同期。次のゲートは必要な設計案の具体化とPR統合であり、本番操作ではない。

rootの既存`runtime`変更を保持し、`origin/main`の`6b45205`から文書専用ブランチ`codex/domain-knowledge-ownership`を分離した。子ソースと本番記録は読取参照とし、既存の証跡ブランチを丸ごと採用しない。独立worker（gpt-6-astra / low）は、文書作成と並行して読取障害のソースに限定した確認を行った。

復旧はこの文書変更のrevertで可能。インストール済み構成・業務データには変更がない。

初版（`6a2395a`）のAIポリシーレビューでは、既読の文書知識・言語・開発方針とPrivate Source Integration Guide、今回のオーナー指示を根拠として文書7ファイルの差分を照合した。既存の「業務スキーマをBase Providerが所有」「Providerは一律非公開」という記述を今回の方針に合わせて修正した。合意した方向と未採用の実装案、公開方針と公開済み状態、業務完了と導入済み状態を区別した。独立workerの読取レビューでも、この境界および障害の説明に重大な指摘はなかった。

初版の変更箇所と新規文書の相対リンク24件は参照先・見出しを確認し、欠落0件。未展開のBase Provider文書への参照1種類は既存checkoutで確認した。新規Mermaid図は5件で、classDiagramの関係線をnamespace外に置くなど構文と責務の表現を目視照合した。GitHub上での描画は未検証。空白差分を検査し、アプリケーションのコードを変更していないためユニットテストは追加・実行していない。

残る判断は本書の未決定案に列挙したマッピング方式・配置・版の結び方・TikTok Web／iOSの公開範囲である。現在の読取障害の根本原因と、業務の正常完了は未確認のまま保持する。

# PR #55のレビュー対応

初版のセルフレビューでは、文書全体のステータスと採用範囲の不整合、移行計画に残る旧配置指示の2点を見落としていた。GitHubのレビュー指摘を根拠に、今回の文書修正で対応した。

| 指摘 | 修正 |
| --- | --- |
| [採用済み境界と未決定案のステータスを分離](https://github.com/flair-agency/live-agency/pull/55#discussion_r3981447987) | 英語正本は採用済みの境界・ドメイン図に限定し、`status: commit`とした。具体的な方式・配置・4図は本書の`pending`な実装案へ移し、相互の参照を更新した |
| [現行移行計画のマッピング所有者も更新](https://github.com/flair-agency/live-agency/pull/55#discussion_r3981447993) | `v2-plan.md`のProfile M2からProviderへの一括移動指示を外し、論理スキーマ・対応定義・実環境bindingの分離へ改訂した。過去の配置指示より今回の方針を優先し、方式と配置は未決定であることを明記した |

修正対象は文書5ファイル。元の5図を保持し、レビュー側の図の説明を日本語にそろえた。移行計画が参照する過去の採用記録は書き換えず、現行計画から後続方針による更新を明示した。独立workerは指摘2件への対応と、移した図の意味・未決定事項の保持を読取確認し、追加の重大な指摘はなかった。

修正文書の相対参照103件は参照先・見出しを確認し、欠落0件。未展開コンポーネントの参照先6種類は既存checkoutで確認した。5図の構造と空白差分も確認した。GitHub上の描画は未検証で、業務コードのテストやサービス操作は実行していない。本番の読取障害はこの文書修正によって解決済みにはしない。
